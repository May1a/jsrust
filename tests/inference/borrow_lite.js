...
export function runInferenceBorrowLiteTests() {
    ...
    assert(
        "borrow-lite: non-copy values can be aliased by reference reuse",
        () => {
            const result = compile(
    ...
    assert(
        "borrow-lite: copyable values can be reused after pass-by-value",
        () => {
            const result = compile(
    ...
    assert(
        "borrow-lite: reject escaping local reference through outer binding",
        () => {
            const result = compile(
    ...
    assert(
        "borrow-lite: allow returning references derived from parameters",
        () => {
    ...
    assert(
        "borrow-lite: nested reference call mismatch reports correct argument type",
        () => {
            const result = compile(
            ...
            assertTrue(
                result.errors[0].message.includes("Argument type mismatch"),
            );
            assertTrue(
                result.errors[0].message.includes("expected &i32, got &&i32"),
            );
        },
    );
