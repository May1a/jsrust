...
function createSimpleFn(name, bodyExpr, returnType = { kind: TypeKind.Int, width: IntWidth.I32 }) {
...
export function testLowerLiteral() {
    ...
    assertTrue(irFn.blocks[0].instructions.length >= 1, 'Should have at least one instruction');
}
...
export function testLowerBinaryAdd() {
    ...
    assertEqual(irFn.blocks[0].instructions.length, 3, 'Should have 3 instructions (2 iconst + 1 iadd)');
}
...
export function testLowerBinarySub() {
    ...
    const hasIsub = irFn.blocks[0].instructions.some(i => i.kind === IRInstKind.Isub);
...
export function testLowerBinaryMul() {
    ...
    const irFn = lowerHirToSsa(fn);
    assertTrue(irFn !== null);
    const hasImul = irFn.blocks[0].instructions.some(i => i.kind === IRInstKind.Imul);
...
export function testLowerBinaryDiv() {
    ...
    const irFn = lowerHirToSsa(fn);
    assertTrue(irFn !== null);
    const hasIdiv = irFn.blocks[0].instructions.some(i => i.kind === IRInstKind.Idiv);
...
export function testLowerBinaryComparison() {
    ...
    const irFn = lowerHirToSsa(fn);
    assertTrue(irFn !== null);
    const hasIcmp = irFn.blocks[0].instructions.some(i => i.kind === IRInstKind.Icmp);
...
export function testLowerIf() {
    ...
    assertTrue(irFn.blocks.length >= 3, 'Should have at least 3 blocks (then, else, merge)');
}
...
export function testLowerWhile() {
    ...
    const irFn = lowerHirToSsa(fn);
    assertTrue(irFn !== null);
    assertTrue(irFn.blocks.length >= 3, 'Should have at least 3 blocks (header, body, exit)');
}
...
export function testLowerLoop() {
    ...
    const irFn = lowerHirToSsa(fn);
    assertTrue(irFn !== null);
    assertTrue(irFn.blocks.length >= 2, 'Should have at least 2 blocks (header, exit)');
}

export function testLowerCall() {
    // foo()
    const callee = makeHVarExpr(span(), 'foo', -1, { kind: TypeKind.Fn, params: [], returnType: intType() });
...
export function testLowerCallWithArgs() {
    ...
    const irFn = lowerHirToSsa(fn);
    assertTrue(irFn !== null);
    const hasCall = irFn.blocks[0].instructions.some(i => i.kind === IRInstKind.Call);
    assertTrue(hasCall, 'Should have call instruction');
}
...
export function testLowerStruct() {
    // Point { x: 1, y: 2 }
    const fields = [
        { name: 'x', value: makeHLiteralExpr(span(), HLiteralKind.Int, 1, intType()) },
        { name: 'y', value: makeHLiteralExpr(span(), HLiteralKind.Int, 2, intType()) }
    ];
    const structTy = { 
        kind: TypeKind.Struct, 
        name: 'Point', 
        fields: [{ name: 'x', type: intType() }, { name: 'y', type: intType() }] 
    };
    ...
    const hasStructCreate = irFn.blocks[0].instructions.some(i => i.kind === IRInstKind.StructCreate);
...
export function testLowerEnum() {
    ...
    const enumTy = { 
        kind: TypeKind.Enum, 
        name: 'Option', 
        variants: [{ name: 'Some', fields: [intType()] }, { name: 'None', fields: [] }] 
    };
    ...
    const hasEnumCreate = irFn.blocks[0].instructions.some(i => i.kind === IRInstKind.EnumCreate);
...
export function testLowerUnaryNeg() {
    ...
    const hasIneg = irFn.blocks[0].instructions.some(i => i.kind === IRInstKind.Ineg);
    assertTrue(hasIneg, 'Should have ineg instruction');
}

export function testLowerUnaryNot() {
    const operand = makeHLiteralExpr(span(), HLiteralKind.Bool, true, boolType());
    ...
    const hasXor = irFn.blocks[0].instructions.some(i => i.kind === IRInstKind.Ixor);
...
export function testLowerFloatLiteral() {
    ...
    const irFn = lowerHirToSsa(fn);
    assertTrue(irFn !== null);
    const hasFconst = irFn.blocks[0].instructions.some(i => i.kind === IRInstKind.Fconst);
...
export function testLowerFloatAdd() {
    ...
    const right = makeHLiteralExpr(span(), HLiteralKind.Float, 2.5, floatType());
    const add = makeHBinaryExpr(span(), BinaryOp.Add, left, right, floatType());
    
    const fn = createSimpleFn('test_fadd', add, floatType());
    const irFn = lowerHirToSsa(fn);
    assertTrue(irFn !== null);
    const hasFadd = irFn.blocks[0].instructions.some(i => i.kind === IRInstKind.Fadd);
...
export function testLowerLetStmt() {
    ...
    const hasIconst = irFn.blocks[0].instructions.some(i => i.kind === IRInstKind.Iconst);
...
export function testLowerBitwiseOps() {
    ...
    const andExpr = makeHBinaryExpr(span(), BinaryOp.BitAnd, left, right, intType());
    
    const fn = createSimpleFn('test_bitand', andExpr);
    const irFn = lowerHirToSsa(fn);
    assertTrue(irFn !== null);
    const hasIand = irFn.blocks[0].instructions.some(i => i.kind === IRInstKind.Iand);
...
export function testLowerMatchLiteral() {
    ...
    const arm1Body = makeHBlock(span(), [], makeHLiteralExpr(span(), HLiteralKind.Bool, true, boolType()), boolType());
    const arm1 = makeHMatchArm(span(), arm1Pat, null, arm1Body);
    
    const arm2Pat = makeHWildcardPat(span(), intType());
    const arm2Body = makeHBlock(span(), [], makeHLiteralExpr(span(), HLiteralKind.Bool, false, boolType()), boolType());
    const arm2 = makeHMatchArm(span(), arm2Pat, null, arm2Body);
    
    const matchExpr = makeHMatchExpr(span(), scrutinee, [arm1, arm2], boolType());
