...
function testLowerFloatLiteral() {
    ...
    assertEqual(hir.kind, HExprKind.Literal, 'Should be a literal expression');
    assertEqual(hir.literalKind, HLiteralKind.Float, 'Should be a float literal');
...
function testLowerStringLiteral() {
    ...
    assertEqual(hir.kind, HExprKind.Literal, 'Should be a literal expression');
    assertEqual(hir.literalKind, HLiteralKind.String, 'Should be a string literal');
...
function testLowerUnboundIdentifier() {
    ...
    assertEqual(hir.kind, HExprKind.Unit, 'Should return unit on unbound identifier');
...
function testLowerBinaryAdd() {
    ...
    const right = makeLiteralExpr(makeSpan(0, 0, 2, 1), LiteralKind.Int, 2, '2');
...
function testLowerBinaryLogical() {
    ...
    const left = makeLiteralExpr(makeSpan(0, 0, 0, 4), LiteralKind.Bool, true, 'true');
    const right = makeLiteralExpr(makeSpan(0, 0, 6, 5), LiteralKind.Bool, false, 'false');
    const ast = makeBinaryExpr(makeSpan(0, 0, 0, 11), BinaryOp.And, left, right);
...
function testLowerUnaryNot() {
    ...
    const operand = makeLiteralExpr(makeSpan(0, 0, 1, 4), LiteralKind.Bool, true, 'true');
...
function testLowerUnaryNeg() {
    ...
    const operand = makeLiteralExpr(makeSpan(0, 0, 1, 1), LiteralKind.Int, 5, '5');
...
function testLowerRefExpression() {
    ...
    const ast = makeRefExpr(makeSpan(0, 0, 0, 2), Mutability.Immutable, operand);
