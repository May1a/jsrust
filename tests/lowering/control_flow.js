...
function testLowerIfSimple() {
    ...
    const condition = makeLiteralExpr(makeSpan(0, 0, 3, 4), LiteralKind.Bool, true, 'true');
    ...
    assertEqual(hir.condition.kind, HExprKind.Literal, 'Should have literal condition');
...
function testLowerIfWithElse() {
    ...
    const ast = makeIfExpr(makeSpan(0, 0, 0, 22), condition, thenBranch, elseBranch);

    const hir = lowerIf(ctx, ast, typeCtx);

    assertEqual(hir.kind, HExprKind.If, 'Should be an if expression');
    assertEqual(hir.elseBranch.kind, HExprKind.Unit, 'Else should be a block (unit expr wrapper)');
}
...
function testLowerIfWithFinalExpr() {
    ...
    const condition = makeLiteralExpr(makeSpan(0, 0, 3, 4), LiteralKind.Bool, true, 'true');
    const thenExpr = makeLiteralExpr(makeSpan(0, 0, 9, 1), LiteralKind.Int, 1, '1');
    const thenBranch = makeBlockExpr(makeSpan(0, 0, 8, 3), [], thenExpr);
    const elseExpr = makeLiteralExpr(makeSpan(0, 0, 19, 1), LiteralKind.Int, 2, '2');
    const elseBranch = makeBlockExpr(makeSpan(0, 0, 18, 3), [], elseExpr);
    const ast = makeIfExpr(makeSpan(0, 0, 0, 23), condition, thenBranch, elseBranch);

    const hir = lowerIf(ctx, ast, typeCtx);

    assertEqual(hir.kind, HExprKind.If, 'Should be an if expression');
    assertEqual(hir.thenBranch.expr.kind, HExprKind.Literal, 'Then should have final expr');
}
...
function testLowerWhileSimple() {
    ...
    const condition = makeLiteralExpr(makeSpan(0, 0, 6, 4), LiteralKind.Bool, true, 'true');
...
function testLowerMatchSimple() {
    ...
    const arm2Pat = makeIdentPat(makeSpan(0, 0, 22, 1), '_', Mutability.Immutable, false, null);
    ...
    assertEqual(hir.arms[0].pat.kind, HPatKind.Literal, 'First arm should have literal pattern');
    assertEqual(hir.arms[1].pat.kind, HPatKind.Ident, 'Second arm should have ident pattern');
}
...
function testLowerMatchWithGuard() {
    ...
    const armPat = makeIdentPat(makeSpan(0, 0, 12, 1), 'x', Mutability.Immutable, false, null);
    ...
    assertEqual(hir.arms[0].guard.kind, HExprKind.Binary, 'Arm should have guard');
}
