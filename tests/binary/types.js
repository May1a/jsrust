...
test("Serialize and deserialize integer types", () => {
    ...
    assertTrue(
        irTypeEquals(
            result.value.functions[0].returnType,
...
test("Serialize and deserialize bool type", () => {
    ...
    assertTrue(result.ok);
    assertTrue(
        irTypeEquals(result.value.functions[0].returnType, makeIRBoolType()),
    );
});
...
test("Serialize and deserialize unit type", () => {
    ...
    assertTrue(result.ok);
    assertTrue(
        irTypeEquals(result.value.functions[0].returnType, makeIRUnitType()),
    );
});
...
test("Serialize and deserialize pointer type", () => {
    ...
    assertTrue(result.ok);
    assertTrue(
        irTypeEquals(result.value.functions[0].returnType, makeIRPtrType(null)),
    );
});
...
test("Serialize and deserialize array type", () => {
    ...
    assertTrue(
        irTypeEquals(
            result.value.functions[0].returnType,
            makeIRArrayType(makeIRIntType(IntWidth.I32), 10),
        ),
    );
});
