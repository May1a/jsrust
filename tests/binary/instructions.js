...
test('Serialize and deserialize binary arithmetic instructions', () => {
    const binaryOps = [
        IRInstKind.Iadd, IRInstKind.Isub, IRInstKind.Imul, IRInstKind.Idiv, IRInstKind.Imod,
        IRInstKind.Fadd, IRInstKind.Fsub, IRInstKind.Fmul, IRInstKind.Fdiv,
        IRInstKind.Iand, IRInstKind.Ior, IRInstKind.Ixor, IRInstKind.Ishl, IRInstKind.Ishr,
    ];
...
test('Serialize and deserialize unary instructions', () => {
    ...
    for (const op of unaryOps) {
        const module = createModuleWithInstructions([{
            kind: op,
            id: 0,
            ty: op === IRInstKind.Ineg ? makeIRIntType(IntWidth.I32) : makeIRFloatType(FloatWidth.F64),
            a: 1,
        }]);
...
test('Serialize and deserialize conversion instructions', () => {
    const convOps = [
        IRInstKind.Trunc, IRInstKind.Sext, IRInstKind.Zext,
        IRInstKind.Fptoui, IRInstKind.Fptosi, IRInstKind.Uitofp, IRInstKind.Sitofp,
        IRInstKind.Bitcast,
    ];
    
    for (const op of convOps) {
        const needsFromTy = [IRInstKind.Trunc, IRInstKind.Sext, IRInstKind.Zext].includes(op);
        const inst = {
            kind: op,
            id: 0,
            ty: makeIRIntType(IntWidth.I32),
        ...
        assertTrue(result.ok, `Failed for op ${op}`);
        const deserializedInst = result.value.functions[0].blocks[0].instructions[0];
