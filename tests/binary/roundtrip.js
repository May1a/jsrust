...
test("Roundtrip function with locals", () => {
    ...
    addIRLocal(
        fn,
        makeIRLocal(1, makeIRPtrType(makeIRIntType(IntWidth.I32)), "ptr"),
    );
    const block = makeIRBlock(0);
    setIRTerminator(block, { kind: IRTermKind.Ret, value: null });
    addIRBlock(fn, block);
...
test("Roundtrip function with instructions", () => {
    ...
    assertEqual(
        deserializedFn.blocks[0].instructions[0].kind,
        IRInstKind.Iconst,
    );
...
test("Roundtrip multiple blocks", () => {
    ...
    assertEqual(
        result.value.functions[0].blocks[0].terminator.kind,
        IRTermKind.Br,
    );
