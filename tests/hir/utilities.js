...
test('hirToString formats integer literal', () => {
    const span = { line: 1, column: 1, start: 0, end: 5 };
    const expr = makeHLiteralExpr(span, HLiteralKind.Int, 42, makeIntType(IntWidth.I32));
    assertEqual(hirToString(expr), '42');
});

test('hirToString formats boolean literal', () => {
    const span = { line: 1, column: 1, start: 0, end: 5 };
    const expr = makeHLiteralExpr(span, HLiteralKind.Bool, true, makeBoolType());
    assertEqual(hirToString(expr), 'true');
});

test('hirToString formats string literal', () => {
    const span = { line: 1, column: 1, start: 0, end: 10 };
    const expr = makeHLiteralExpr(span, HLiteralKind.String, 'hello', makeUnitType());
...
test('hirToString formats binary expression', () => {
    const span = { line: 1, column: 1, start: 0, end: 10 };
    const left = makeHLiteralExpr(span, HLiteralKind.Int, 1, makeIntType(IntWidth.I32));
    const right = makeHLiteralExpr(span, HLiteralKind.Int, 2, makeIntType(IntWidth.I32));
    const expr = makeHBinaryExpr(span, 0, left, right, makeIntType(IntWidth.I32));
    assertEqual(hirToString(expr), '(1 + 2)');
});

test('hirToString formats unary expression', () => {
    const span = { line: 1, column: 1, start: 0, end: 5 };
    const operand = makeHLiteralExpr(span, HLiteralKind.Int, 5, makeIntType(IntWidth.I32));
...
test('hirToString formats call expression', () => {
    const span = { line: 1, column: 1, start: 0, end: 10 };
    const callee = makeHVarExpr(span, 'foo', 0, makeFnType([], makeUnitType(), false));
...
test('hirToString formats call expression with args', () => {
    const span = { line: 1, column: 1, start: 0, end: 10 };
    const callee = makeHVarExpr(span, 'add', 0, makeFnType([], makeUnitType(), false));
    const arg1 = makeHLiteralExpr(span, HLiteralKind.Int, 1, makeIntType(IntWidth.I32));
    const arg2 = makeHLiteralExpr(span, HLiteralKind.Int, 2, makeIntType(IntWidth.I32));
    const expr = makeHCallExpr(span, callee, [arg1, arg2], makeIntType(IntWidth.I32));
...
test('hirToString formats struct expression', () => {
    ...
    const value = makeHLiteralExpr(span, HLiteralKind.Int, 10, makeIntType(IntWidth.I32));
    const expr = makeHStructExpr(span, 'Point', [{ name: 'x', value }], null, makeUnitType());
    assertEqual(hirToString(expr), 'Point { x: 10 }');
});

test('hirToString formats enum expression', () => {
    const span = { line: 1, column: 1, start: 0, end: 15 };
    const field = makeHLiteralExpr(span, HLiteralKind.Int, 42, makeIntType(IntWidth.I32));
    const expr = makeHEnumExpr(span, 'Option', 'Some', 1, [field], makeUnitType());
...
test('hirToString formats if expression', () => {
    ...
    const cond = makeHLiteralExpr(span, HLiteralKind.Bool, true, makeBoolType());
    const thenBlock = makeHBlock(span, [], makeHUnitExpr(span, makeUnitType()), makeUnitType());
...
test('hirToString formats loop expression', () => {
    const span = { line: 1, column: 1, start: 0, end: 20 };
    const body = makeHBlock(span, [], makeHUnitExpr(span, makeUnitType()), makeUnitType());
...
test('hirToString formats let statement', () => {
    const span = { line: 1, column: 1, start: 0, end: 15 };
    const pat = makeHIdentPat(span, 'x', 0, makeIntType(IntWidth.I32), false, false);
    const init = makeHLiteralExpr(span, HLiteralKind.Int, 42, makeIntType(IntWidth.I32));
...
test('hirToString formats return statement', () => {
    const span = { line: 1, column: 1, start: 0, end: 10 };
    const value = makeHLiteralExpr(span, HLiteralKind.Int, 42, makeIntType(IntWidth.I32));
...
test('hirToString formats field place', () => {
    ...
    const place = makeHFieldPlace(span, base, 'x', 0, makeIntType(IntWidth.I32));
...
test('hirToString formats literal pattern', () => {
    const span = { line: 1, column: 1, start: 0, end: 5 };
    const pat = makeHLiteralPat(span, HLiteralKind.Int, 42, makeIntType(IntWidth.I32));
    assertEqual(hirToString(pat), '42');
});

test('hirToString formats struct pattern', () => {
    const span = { line: 1, column: 1, start: 0, end: 20 };
    const fieldPat = makeHIdentPat(span, 'x', 0, makeIntType(IntWidth.I32), false, false);
    const pat = makeHStructPat(span, 'Point', [{ name: 'x', pat: fieldPat }], false, makeUnitType());
    assertEqual(hirToString(pat), 'Point { x: x }');
});

test('hirToString formats tuple pattern', () => {
    const span = { line: 1, column: 1, start: 0, end: 15 };
    const elem = makeHIdentPat(span, 'x', 0, makeIntType(IntWidth.I32), false, false);
...
test('hirToString formats or pattern', () => {
    const span = { line: 1, column: 1, start: 0, end: 10 };
    const left = makeHLiteralPat(span, HLiteralKind.Int, 1, makeIntType(IntWidth.I32));
    const right = makeHLiteralPat(span, HLiteralKind.Int, 2, makeIntType(IntWidth.I32));
...
test('hirToString formats function declaration', () => {
    const span = { line: 1, column: 1, start: 0, end: 20 };
    const fn = makeHFnDecl(span, 'main', null, [], makeUnitType(), null, false, false);
...
test('hirToString formats enum declaration', () => {
    ...
    const variant2 = makeHEnumVariant(span, 'Some', [makeIntType(IntWidth.I32)], null);
...
test('collectVars finds variable in pattern', () => {
    const span = { line: 1, column: 1, start: 0, end: 5 };
    const pat = makeHIdentPat(span, 'x', 5, makeIntType(IntWidth.I32), false, false);
...
test('collectVars finds variables in if expression', () => {
    ...
    const thenBlock = makeHBlock(span, [], makeHVarExpr(span, 'x', 2, makeIntType(IntWidth.I32)), makeIntType(IntWidth.I32));
    const expr = makeHIfExpr(span, cond, thenBlock, null, makeIntType(IntWidth.I32));
    const vars = collectVars(expr);
    assertTrue(vars.has(1));
    assertTrue(vars.has(2));
});
...
test('collectVars finds variables in match expression', () => {
    ...
    const pat = makeHIdentPat(span, 'y', 2, makeIntType(IntWidth.I32), false, false);
    const body = makeHBlock(span, [], makeHVarExpr(span, 'y', 2, makeIntType(IntWidth.I32)), makeIntType(IntWidth.I32));
    const arm = makeHMatchArm(span, pat, null, body);
    const expr = makeHMatchExpr(span, scrutinee, [arm], makeIntType(IntWidth.I32));
    const vars = collectVars(expr);
    assertTrue(vars.has(1));
    assertTrue(vars.has(2));
});
...
test('collectVars finds variables in function declaration', () => {
    ...
    const fn = makeHFnDecl(span, 'get_x', null, [], makeIntType(IntWidth.I32), body, false, false);
