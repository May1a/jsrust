...
test('makeHIndexPlace creates index place', () => {
    ...
    const index = makeHLiteralExpr(span, HLiteralKind.Int, 0, makeIntType(IntWidth.I32));
...
test('makeHLiteralExpr creates string literal', () => {
    const span = { line: 1, column: 1, start: 0, end: 10 };
    const expr = makeHLiteralExpr(span, HLiteralKind.String, 'hello', makeStringType());
...
test('makeHIfExpr creates if-else expression', () => {
    ...
    const cond = makeHLiteralExpr(span, HLiteralKind.Bool, true, makeBoolType());
    const thenBlock = makeHBlock(span, [], makeHUnitExpr(span, makeUnitType()), makeUnitType());
    const elseBlock = makeHBlock(span, [], makeHUnitExpr(span, makeUnitType()), makeUnitType());
...
test('makeHIdentPat creates mutable pattern', () => {
    const span = { line: 1, column: 1, start: 0, end: 10 };
    const pat = makeHIdentPat(span, 'x', 0, makeIntType(IntWidth.I32), true, false);
...
test('makeHTuplePat creates tuple pattern', () => {
    ...
    const pat = makeHTuplePat(span, [elem], makeTupleType([makeIntType(IntWidth.I32)]));
...
test('isHItem correctly identifies items', () => {
    const span = { line: 1, column: 1, start: 0, end: 20 };
    const fn = makeHFnDecl(span, 'test', null, [], makeUnitType(), null, false, false);
    const expr = makeHUnitExpr(span, makeUnitType());
