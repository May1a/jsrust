...
test('undefined value in instruction', () => {
    ...
    assertTrue(result.errors.some(e => e.kind === ValidationErrorKind.UndefinedValue));
});
...
test('missing terminator', () => {
    ...
    const result = validateModule(mod);
    assertTrue(!result.ok, 'Module should be invalid');
    assertTrue(result.errors.some(e => e.kind === ValidationErrorKind.MissingTerminator));
});
...
test('branch to undefined block', () => {
    ...
    const result = validateModule(mod);
    assertTrue(!result.ok, 'Module should be invalid');
    assertTrue(result.errors.some(e => e.kind === ValidationErrorKind.UndefinedBlock));
});
...
test('duplicate function names', () => {
    ...
    const result = validateModule(mod);
    assertTrue(!result.ok, 'Module should be invalid');
    assertTrue(result.errors.some(e => e.kind === ValidationErrorKind.DuplicateDefinition));
});

test('unreachable block', () => {
    resetIRIds();
    const mod = makeIRModule('test');
    const fn = makeIRFunction(0, 'unreachable', [], makeIRIntType(IntWidth.I32));
    ...
    const result = validateModule(mod);
    assertTrue(!result.ok, 'Module should be invalid');
    assertTrue(result.errors.some(e => e.kind === ValidationErrorKind.UnreachableBlock));
});
