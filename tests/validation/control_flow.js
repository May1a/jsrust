...
test('valid switch', () => {
    resetIRIds();
    const mod = makeIRModule('test');
    const fn = makeIRFunction(0, 'switch_test', [], makeIRIntType(IntWidth.I32));
...
test('branch argument count mismatch', () => {
    ...
    assertTrue(result.errors.some(e => e.kind === ValidationErrorKind.InvalidBlockArg));
});
