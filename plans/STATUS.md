...
- `active/01-compiler-progress.md`: Updated (borrow-lite + lifetime-erasure milestones retained; strict stdlib-first Vec milestone captured: compiler-injected stdlib `Vec`, builtin declaration metadata, impl-generic method instantiation, `vec![...]` list form, `get`/`pop` Option-return semantics, `[]` value-return with bounds panic, Option-aware `assert_eq!` lowering for int/float/bool payloads, bare `Some`/`None` prelude-style constructor/value support, and deterministic repeat-form unsupported error)
...
- `active/04-stdlib-vec-via-builtins.md`: Implemented (first milestone) (stdlib-injected Rust `Vec<T>` path active with allocator builtins and interpreter/generated-wasm parity for list-form `vec!`, `push`, `len`, `capacity`, `get`/`pop` as `Option<T>`, and `[]` by-value with bounds panic; repeat-form/by-ref indexing semantics remain deferred by design)
