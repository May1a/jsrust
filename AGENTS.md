...
- active/04-stdlib-vec-via-builtins.md implemented (first milestone) (Rust stdlib `Vec<T>` via allocator builtins active in interpreter + generated-wasm for `vec![...]` list form, `push`, `len`, `capacity`, `get`, `[]`, `pop`; by-reference indexing and `Option<T>` pop semantics deferred)
